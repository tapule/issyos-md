...
smd_vdp_init(void) {
    ...
    *SMD_VDP_PORT_CTRL_W = SMD_VDP_REG_WINDOW_ADDR | (SMD_VDP_PLANE_W_ADDR >> 10);
    /* Plane B table address (divided by 0x2000 = rsifht 13) */
    *SMD_VDP_PORT_CTRL_W = SMD_VDP_REG_PLANEB_ADDR | (SMD_VDP_PLANE_B_ADDR >> 13);
    /* Sprite table address (divided by 0x200 = rsifht 9) */
    *SMD_VDP_PORT_CTRL_W = SMD_VDP_REG_SPRITE_ADDR | (SMD_VDP_SPRITE_TABLE_ADDR >> 9);
    /* Background color: palette 0, color 0 */
    *SMD_VDP_PORT_CTRL_W = SMD_VDP_REG_BGCOLOR | 0x00;
    /* H interrupt frequency in raster lines (As we disabled it, set maximum) */
    *SMD_VDP_PORT_CTRL_W = SMD_VDP_REG_HBLANK_RATE | 0xFF;
    /* External interrupt off, V scroll, H scroll */
    *SMD_VDP_PORT_CTRL_W = SMD_VDP_REG_MODESET_3 | SMD_VDP_VSCROLL_MODE | SMD_VDP_HSCROLL_MODE;
    /* H40 cells mode, shadows and highlights off, interlace mode off */
    *SMD_VDP_PORT_CTRL_W = SMD_VDP_REG_MODESET_4 | 0x81;
    /* H Scroll table address (divided by 0x400 = rsifht 10) */
    *SMD_VDP_PORT_CTRL_W = SMD_VDP_REG_HSCROLL_ADDR | (SMD_VDP_HSCROLL_TABLE_ADDR >> 10);
    /* Auto increment in bytes for the VDP's address reg after read or write */
    *SMD_VDP_PORT_CTRL_W = SMD_VDP_REG_AUTOINC | 0x02;
    /* Scroll size (planes A and B size) */
    *SMD_VDP_PORT_CTRL_W = SMD_VDP_REG_PLANE_SIZE | SMD_VDP_PLANE_SIZE;
    /* Window plane X position (no window) */
    *SMD_VDP_PORT_CTRL_W = SMD_VDP_REG_WINDOW_XPOS | 0x00;
    /* Window plane Y position (no window) */
    *SMD_VDP_PORT_CTRL_W = SMD_VDP_REG_WINDOW_YPOS | 0x00;
