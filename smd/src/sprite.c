...
inline void smd_spr_init(void)
{
    smd_spr_clear();
}

inline uint16_t smd_spr_attributes_encode(const uint16_t priority,
...
inline void smd_spr_attributes_vflip(uint16_t* attributes)
{
    *attributes ^= 0x1000;
}

inline uint16_t smd_spr_attributes_vflipb(uint16_t attributes)
{
    return attributes ^ 0x1000;
}

inline void smd_spr_attributes_hflip(uint16_t* attributes)
{
    *attributes ^= 0x0800;
}

inline uint8_t smd_spr_size_encode(const uint8_t width, const uint8_t height)
{
    return (((height - 1) & 0x03) | (((width - 1) & 0x03) << 2));
}

inline uint8_t smd_spr_size_to_tiles(const smd_spr_size_t size)
{
...
inline void smd_spr_clear(void)
{
    smd_spr_count = 0;
...
void smd_spr_update(void)
{
    ...
    dma_vram_transfer_fast(smd_spr_table, SMD_VDP_SPRITE_TABLE_ADDR, smd_spr_count << 2, 2);
