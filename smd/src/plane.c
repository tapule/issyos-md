...
{
    dma_vram_fill(plane, SMD_VDP_PLANE_TILES << 1, 0x00, 1);
}
                     ...
                     const uint16_t height)
{
    ...
    for (i = 0; i < height; ++i)
    {
        ...
        dma_vram_transfer_fast(&tile_row,
                               plane + ((x + ((y + i) * SMD_VDP_PLANE_WIDTH)) << 1),
                               width, 2);
    }
}
                     ...
                     const uint16_t x, const uint16_t y)
{
    ...
    *SMD_VDP_PORT_CTRL_L = (((uint32_t)(SMD_VDP_VRAM_WRITE_CMD)) |
                      ...
                      const bool defer)
{
    if (defer)
    {
        dma_queue_vram_transfer(tiles,
                                plane + ((x + (y * SMD_VDP_PLANE_WIDTH)) << 1),
                                length, 2);
    }
    else
    {
        dma_vram_transfer(tiles, plane + ((x + (y * SMD_VDP_PLANE_WIDTH)) << 1),
                          length, 2);
    }
}
...
{
    dma_vram_transfer_fast(tiles, plane + ((x + (y * SMD_VDP_PLANE_WIDTH)) << 1),
                           length, 2);
}
                      ...
                      const bool defer)
{
    if (defer)
    {
        dma_queue_vram_transfer(tiles,
                                plane + ((x + (y * SMD_VDP_PLANE_WIDTH)) << 1),
                                length, SMD_VDP_PLANE_WIDTH << 1);
    }
    else
    {
        dma_vram_transfer(tiles,
                          plane + ((x + (y * SMD_VDP_PLANE_WIDTH)) << 1), length,
    ...
    if (defer)
    {
        for (row = 0; row < height; ++row)
        {
            dma_queue_vram_transfer(tiles + (row * width),
                             plane + ((x + ((y + row) * SMD_VDP_PLANE_WIDTH)) << 1),
                             width, 2);
        }
    }
    else
